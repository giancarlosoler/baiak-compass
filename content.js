...
(() => {
  ...
  function readMeasurementTotals() {
    ...
    // Si el panel del Hunt Analyzer no está en el DOM (transición de UI, modal encima, etc.)
    // la lectura es inválida: no debe confundirse con un reinicio real de XP Gain a 0.
    const valid = !!document.querySelector("#an-raw, #panel-hunt");
    return {
      valid,
  ...
  // Re-rankea usando la party y los logos ya capturados en el último analyze() real,
  // sin reabrir el selector de hunts ni volver a clicar cada miembro de la party.
  // Se usa tras cada calibración de 5 minutos, donde solo cambió huntMeasurements.
  async function reRankWithMeasurements(measurements) {
    const fresh = await chrome.storage.local.get(DEFAULTS);
    const party = fresh.lastAnalysis?.party || [];
    if (!party.length) return;
    const hunts = parseHunts(fresh.huntLogos || {}).map(h => ({
      ...h,
      image: preferredHuntImage(h.image, fresh.huntLogos?.[norm(h.name)])
    }));
    if (!hunts.length) return;
    const settings = {...fresh, huntMeasurements: measurements};
    const ranked = normalizeScores(hunts.map(h => scoreHunt(h, party, settings, fresh.learned || {})));
    const result = {...(fresh.lastAnalysis || {}), at: Date.now(), ranked, party};
    await chrome.storage.local.set({lastAnalysis: result});
  }
  ...
  async function measurementTick() {
    ...
    if (!hunt || normalized === "online training") {
      const {measurementState:pausedState} = await chrome.storage.local.get({measurementState:null});
      if (pausedState?.status === "running") await chrome.storage.local.set({measurementState:{...pausedState,status:"paused",pausedAt:now,updatedAt:now}});
      return;
    }

    // Lectura inválida: no tocar el estado para no confundir un fallo de lectura del DOM
    // con un reinicio real del contador de XP Gain.
    if (!totals.valid) return;

    let {measurementState: state} = await chrome.storage.local.get({measurementState:null});
    ...
    if (elapsedMs >= MEASURE_MS) {
      ...
      const {huntMeasurements, lastAnalysis} = await chrome.storage.local.get({huntMeasurements:{}, lastAnalysis:null});
      const measurements = {...(huntMeasurements || {})};
      const old = measurements[hunt] || {sessions:[]};
      const estimateRow=lastAnalysis?.ranked?.find(r=>norm(r.name)===norm(hunt));
      ...
      measurements[hunt] = {sessions,totalSessions,totalXpGainForCalibration:totalXpGain,totalRealXph,totalRealGoldph,totalGoldGain,
        ...
        updatedAt:now,source:"xp-gain-5m-calibration-20pct"};
      ...
      // Re-rankea con los datos ya disponibles: evita reabrir el selector de hunts y volver
      // a capturar la party (clic por clic) cada 5 minutos en pleno combate.
      try { await reRankWithMeasurements(measurements); } catch (_) {}
  ...
  function bossCellData(el){
    ...
    // Se busca también en TODO el texto de la tarjeta como respaldo del selector específico.
    const cellText=(el?.textContent||'').replace(/\s+/g,' ').trim();
  ...
  async function persistBossRun(){
    ...
    // activa, ownBossRun.queue está en su valor inicial [] y guardarlo acá
    // borraría la lista del usuario sin que él lo haya pedido.
    const patch={bossOwnRun:{state:ownBossRun.state,index:ownBossRun.index,total:ownBossRun.total,current:ownBossRun.current,message:ownBossRun.message,startedAt:ownBossRun.startedAt}};
    if(Array.isArray(ownBossRun.queue)&&ownBossRun.queue.length) patch.bossOwnQueue=ownBossRun.queue;
  ...
  async function goToRecommendedHunt() {
    let analysis;
    try { analysis = await analyze(); }
    catch (_) { const st = await chrome.storage.local.get({ lastAnalysis: null }); analysis = st.lastAnalysis; }
    const best = analysis?.ranked?.[0];
    if (!best?.name) return null;
    await switchToHunt(best.name);
    return best.name;
  }
  async function runOwnBossQueue(queue){
    ...
    try{
      ...
      const parts=[];
      if(tally.completed) parts.push(`${tally.completed} completados`);
      if(tally.already_completed) parts.push(`${tally.already_completed} ya completados`);
      if(tally.cooldown) parts.push(`${tally.cooldown} en cooldown`);
      if(tally.rejected) parts.push(`${tally.rejected} rechazados`);
      if(tally.defeat) parts.push(`${tally.defeat} derrotas`);
      if(tally.error) parts.push(`${tally.error} con error`);
      const summary=parts.length?` (${parts.join(', ')})`:'';
      ownBossRun.state=ownBossRun.stop?'stopped':'finished';ownBossRun.current='';
      if(!ownBossRun.stop){
        let huntName=null;
  ...
  chrome.storage.local.get({bossCatalogCache:{bosses:[],charges:'',updatedAt:0},huntLogos:{},huntMeasurements:{}}).then(async st=>{
    ...
    // Migración única: instalaciones previas guardaban el logo (data URL) duplicado en cada
    // sesión de medición y en cada entrada de huntMeasurements. Ya está en huntLogos, así
    // que se retira aquí para liberar espacio de storage.
    let changedMeasurements=false;
    const cleanedMeasurements={};
    for(const [huntName,entry] of Object.entries(st.huntMeasurements||{})){
      const {image,sessions,...rest}=entry||{};
      if(image!==undefined) changedMeasurements=true;
      const cleanedSessions=(sessions||[]).map(s=>{
        if(s?.image===undefined) return s;
        changedMeasurements=true;
        const {image:_sessionImage,...sessionRest}=s;
        return sessionRest;
      });
      cleanedMeasurements[huntName]={...rest,sessions:cleanedSessions};
    }
    if(changedMeasurements) await chrome.storage.local.set({huntMeasurements:cleanedMeasurements}).catch(()=>{});
  }).catch(()=>{});
  chrome.runtime.onMessage.addListener((msg,_sender,send)=>{
    (async()=>{
      ...
      else if(msg.type==="BAIAK_BOSS_STOP") { if(ownBossRun.state==="running"||ownBossRun.state==="paused"){ ownBossRun.stop=true; ownBossRun.pause=false; ownBossRun.state='stopped'; await persistBossRun(); } send({ok:true,message:'Auto Boss detenido.'}); }
      else send({ok:false,error:`Mensaje desconocido: ${msg.type}`});
